...
exec("meteor build build --architecture os.linux.x86_64", (e,se,so) => {
	...
	if(!e) {
		...
		exec("scp "+process.cwd()+"/build/"+config.local_app_name+".tar.gz "+config.wf_username+"@"+config.wf_server+":/home/"+config.wf_username+"/webapps/"+config.wf_app_name+"/build.tar.gz && rm -rf "+process.cwd()+"/build/", (e, se, so) => {
			console.log(so);
			...
			if(!e) {
				...
				exec("ssh "+config.wf_username+"@"+config.wf_server+" \"cd ~/webapps/"+config.wf_app_name+" && rm -rf bundle && tar -zxf build.tar.gz && rm build.tar.gz\"", (e,se,so) => {
					...
					if(!e) {
						...
						console.log("Files extracted. Installing app dependencies with npm install...");
							...
							if(!e) {
								...
								exec("scp start "+config.wf_username+"@"+config.wf_server+":/home/bvodola/webapps/"+config.wf_app_name+"/bin/start", (e,se,so) => {
									...
									if(!e) {
										...
										exec("ssh "+config.wf_username+"@"+config.wf_server+" \"cd ~/webapps/"+config.wf_app_name+" && export MONGO_URL="+config.mongo_url+" && export ROOT_URL="+config.wf_root_url+" && export PORT="+config.wf_port+" && cd bin && ./stop && chmod u+x start && ./start \"", (e,se,so) => {
											if(!e) {
